...
import logotemp2 from '../../images/logotemp2.png'
...
const Navbar = () => {
  ...
  return (
    <AppBar className={classes.appBar} position="static" color="inherit">
      ...
      <Typography component={Link} to="/about" className={classes.donation} variant="h5" align="center">About Us</Typography>
      ...
      <Toolbar className={classes.toolbar}>
        {user?.result ? (
          <div className={classes.profile}>
            <Avatar component={Link} to={`/creators/${user?.result?.googleId || user?.result?._id}/${user?.result?.name}`} className={classes.purple} alt={user?.result.name} src={user?.result.imageUrl}>{user?.result.name.charAt(0)}</Avatar>
